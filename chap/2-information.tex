...
$f(\cdot;\bth)$ is the true data generating density. This result naturally
suggests one to find the estimator $\tbth$ which minimize the \kl. Under
suitable conditions, rewrite equation~\eqref{eq:kld},
\begin{align}
  \dkl(g,f(\cdot;\bth))
...
\begin{equation}
  ...
  \overset{\mathrm{a.s.}}{\longrightarrow} \Exp_g\log f(X;\bth), \quad\text{for } n\to\infty,
  \label{eq:kl consistency}
\end{equation}
provided that this expectation exits. Hence,
\begin{equation}
...
The general formula of the Akaike's information criterion, namely \aic, for a
...
There are precise mathematical reasons bind the \aic's way of penalizing
complexity and measuring fit of data. These are closed related to the behavior
of the \mle and its relation to the \textcite{Kullback1951}'s measure of the
statistical divergence from one probability density to another. In the next
subsection we outline the derivation of \aic. This shall provide us a basis
for the discussion of the suitability of the \aic and related methods for
model selection.
...
As pointed out by \textcite{Bozdogan1987}, though the development of \aic had
...
essence to estimate the expected \kl from $f(\cdot; \hbth)$ to $g$. Recall
...
this setting a general case in data analysis as argued by Akaike. Instead, in
realistic cases the difficulties of model selection problem often come from
...
much higher noise and it seems to violated the assumption of \aicc even more.
