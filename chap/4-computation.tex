...
\parencite[see][sec.~3.3]{Robert:2004tn}. Also other Bayesian inference problems,
...
only if, \parencite[see][sec.~3.3]{Robert:2004tn},
\begin{equation}
...
\textcite{Geweke:1989tm}. In addition a more critical problem is that, in order
...
improved \parencite{Casella:1998tj}. As suggest in
\textcite[][sec.~3.3]{Robert:2004tn}, to minimize the variance of the estimator
...
to $f(D|\bth)$ \parencite{Kass:1995vb}. And the above equation suggests that
the estimator has a finite variance and thus satisfies the Central Limit
Theorem if the tails of $\gamma$ is thin enough compared to the unnormalized
posterior distribution $f(D|\bth)\pi(\bth)$. \textcite{Gelfand:1994ux} suggested
using multivariate normal distributions with moments estimated from the
posterior samples as a natural choice of $\gamma$. In addition the variance of
the estimator can also be estimated for $1/\widehat{\Pr(D)}$ from the
...
\textcite{Zhou:2011uo}. It was proved to perform well enough for our purpose
and it was relative easy to compute. However, the choice of a sensible density
$\gamma$ is not always easy in general. The multivariate normal or $t$
densities as proposed by \textcite{Gelfand:1994ux} is not always justified. Since
...
stabilizing the harmonic mean estimator. For example
\textcite{Hesterberg:1995wg} proposed using a mixture of the posterior density
$\pi^*(\bth|D)$ and a heavy tailed density, say $\omega(\bth)$,
\[
  (1-\rho)\pi^*(\bth|D) + \rho\omega(\bth),\quad \rho\ll 1,
\]
as the importance density \parencite[see also][]{Owen:2000kb}.
\textcite{Raftery:2006ty} also discussed a method for improving the harmonic
mean by parameter reduction. This method requires analytical form for
integrating at least some parameter out of the unnormalized posterior
distribution. Such analysis is in general intractable for complex models,
especially for irregular likelihood function. \textcite{Chib:1995em} and
\textcite{Chib:2001gq} considered specified methods for estimating the marginal
likelihood when the posterior samples come from Gibbs sampling and \mha,
respectively. The former requires the full conditional density of the
parameters. Again this can be intractable for complex models. And the latter
one inherit the same limitations as the \mha, which will be a subject of the
next subsection.
...
discussions of algorithms. \textcite[][chap.~6]{Robert:2004tn} is a compact
reference for many results of Markov chains related particularly to \mcmc
algorithms. The last not least, the classical paper by
\textcite{Tierney:1994uk} has two sections of key results of Markov chains in
the setting of \mcmc algorithms. These references provide all the results we
...
definitions and treatment can be found in \textcite[][chap.~6]{Robert:2004tn} and
...
\parencite{Geman:1993bp}. We will review these two techniques in the following
...
\parencite[see][chap.~7]{Robert:2004tn}. Instead of discuss the general form of
\mha in details, we focus on some practical implementations of this algorithm
\parencite[also see][for the speed of convergence of the
\mha]{Mengersen:1996th}.
...
see)(){Gilks:1998dl,Haario:1999dh,Haario:2001gu} On the practical side, the
...
met. \textcite{Chib:2001gq} demonstrated a method of computing the marginal
...
\parencite[see][chap.~10]{Robert:2004tn}. However it has several features making
...
\begin{enumerate}
  ...
  \item The Gibbs sampler does not apply to problems where the number of
    parameters varies \parencite[see][chap.~10]{Robert:2004tn}.
\end{enumerate}
...
the same methods in \textcite{Chib:2001gq} can be applied. In fact, this method
was first developed for Gibbs sampling in \textcite{Chib:1995em}.
...
The \emph{pseudo-priors} method was introduced by \textcite{Carlin:1995uy} by
...
\end{equation}
Similar development can also be found in \textcite{Brooks:2003ko,Besag:2001um} based
on the saturation scheme. It is very clear that \textcite{Carlin:1995uy}'s method
...
parameter space $\bTh_k$ may be omitted
\parencites[see][chap.~7]{Robert:2007tc}.
