\chapter{New method for Bayesian computation: using sequential Monte Carlo and
  Path sampling}
\chaptermark{New method for Bayesian computation}
...
distribution. \parencite{DelMoral:2006hc}
...
approximate the estimator. \parencite{Gelman:1998ei}
...
some literature \parencite[e.g.][]{DelMoral:2006hc}.
...
approximation to the distribution $\pi_n$ as possible \parencite{Douc:2005wa}.
...
such that $\sum_{i=1}^N N_n^{(i)} = N$, $\Exp[N_n^{(i)}|X_{1:n}] = NW_n^{(i)}$
and the new particle set has equal weights \parencite{Douc:2005wa,DelMoral:2006hc}.
...
time $n$ \parencite{Liu:1998iu}. Intuitively, with the discrepancy between
...
in \textcite{Liu:1998iu} that the effective sample size (\ess) is
approximately proportional to the inverse of the variance of importance
sampling estimator associated with the weights. And therefore it is used as an
indicator of requirement of resampling.

\paragraph{Summary of the algorithm} In short the algorithm for a sequence
$\{\pi_n\}$, $n = 1,\dots,p$, can be represented as below,
\parencite{DelMoral:2006hc}
\begin{enumerate}
...
\parencite[e.g.][]{West:1993us}. Independent proposals appear to be dependent
on good behavior of the sequence $\{\pi_n\}$. One way to construct proposals
adaptively is proposed by \textcite{Jasra:2008bb}. However, it is still
difficult to construct independent proposals which approximate $\pi_n$ well in
many cases. For example, with a multi modes $\pi_n$, a standard distribution
like Normal can hardly be efficient. Therefore, it is sensible to construct
local moves.

\paragraph{Local random-walk moves} An alternative to above is using
$K_n(x_{n-1},x_n)$ a random-walk kernel \parencite[e.g.][]{Givens:1996ul}. It
...
\textcite{DelMoral:2006hc}, it is natural to set $K_n$ as an \mcmc kernel of
invariant distribution $\pi_n$. It is sensible in the cases either $K_n$ mixes
fast enough or $\pi_n$ evolves slowly so that we can expect $\eta_n$ to be
close to $\pi_n$. The massive amount of literatures in \mcmc, for example some
references mentioned in chapter~\ref{cha:Bayesian computation with Monte Carlo
  methods} can be used to construct efficient \mcmc kernels that mixes fast
enough. This is one of the advantage that make this approach attractive.
Another property of this kind of kernels is shown in the next subsection when
we discuss the choice of backward kernels.
...
Indeed, as shown in \textcite{DelMoral:2006hc}, the optimal backward kernel in
...
\textcite{DelMoral:2006hc}. The one of most interest to this report is as below.
...
performed at each iteration \parencite{DelMoral:2006hc}.
...
harmonic mean estimator described in section~\ref{sub:Importance sampling and
  harmonic mean based estimators} \parencite[see][]{Gelman:1998ei}. It was argued
...
\paragraph{Estimating means of distributions} Just to illustrate how the \smc
...
\paragraph{Quality of the estimator} In \textcite{DelMoral:2006hc} and references
...
\begin{enumerate}
  ...
  \item The theoretical properties of the estimator. This will be a more
    ...
    based on results in \textcite{DelMoral:2006hc}. One of the main interest here
