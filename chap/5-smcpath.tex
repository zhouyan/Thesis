...
particles are moved into high density region of $\pi_n$.
...
impossible in many situations. Note that
\begin{equation}
...
be difficult to compute $K_j$ point-wise, leave alone the integration. The
\smc algorithm introduce an auxiliary backward Markov chain kernel to solve
...
$\gamma_t(\theta)$ known up to normalizing constants, say $Z_t$, where $t$ is
a continuous index taken value on $[0,1]$. The path sampling estimator of the
logarithm of ratio of normalizing constants is based on,
\begin{equation}
...
\end{align}
where the expectation is taken with respect to $\theta$ with $t$ fixed.  Thus,
the path sampling estimator of $\log(Z_1/Z_0)$ can be written as,
\begin{equation}
...
space of $(T, \Theta)$ and use the simple Monte Carlo approximation. An
alternative way is to use a sequence $0 \le t_1 < \dots < t_m \le 1$, and
numerical integration with trapezoidal rule. In other words, a sequence of
distributions $\gamma_{t_n}$ are constructed. Their expectations can be
approximated using the importance distributions from a \smc sampler as
described in section~\ref{sec:smc algorithm}. This later method is the main
...
\begin{remark}
  ...
  \end{align*}
  The value of $\alpha$ and $\beta$ are chosen mainly fro numerical stability
  reason. If $\alpha = 0.01$ or smaller is chosen, then the majority of
  $\lambda$ sampled for the initial distribution will be zero or subnormal.
  The value of $n_0$ is chosen such that the prior distribution is relatively
  flat compared to the posterior distribution. And the \mha bandwidth
  $\sigma_{\mu}$ and $\sigma_{\lambda}$ are chosen such that the acceptance
  rates are within a reasonable range (around $30\%$).
\end{remark}
...
algorithm with $N$ iterations. The main factor is that increasing the number
of distributions in \smc sampler will improve the numerical integration of
path sampling estimator. It will be of interest to investigate how to
construct the ``path'' effectively. And literatures in numerical integration
is a possible source for inspiration. Further, in the simple examples, the
same \mha is used for each iteration when doing the forward moves. However, it
will probably be more efficient to construct the forward kernel adaptively.
