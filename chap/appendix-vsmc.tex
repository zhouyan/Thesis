...
\draftnote{Removed the statement that "class template is simpler". I still
  believe it is true. Class template ask you to specify the template parameter
  and I never had troubles with its features, including specialization.
  Function template, though allows it, usually depends on reduction, which
  when interacted with overloading and specialization and many other C++
  features that are supposed to be there to help, has cost me countless
  sleepless nights. In fact, most bugs I fixed in various C++ projects (e.g.,
  Rcpp) are about function templates.}
