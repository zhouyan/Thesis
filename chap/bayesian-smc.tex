...
At time $t = 0$, sample $\{X_0^{(i)}\}_{i=1}^N$ from $\eta_0$ and compute the
...
\draftnote{I remember some interesting presentations on parallel resampling
  from the SMC workshop. Not sure if anything has been published in journals}

\subsection[SMC samplers]{\protect\smc samplers}
\label{sub:SMC Samplers}

\smc samplers generalizes the \sis algorithm for a sequence of distributions
...
\draftnote{Add a definition of the marginal $\eta_t(x_t)$ which include both
  the no resampling and resampling in the past cases. There might be some
  inconsistency here with the use of $\eta_t(x_{0:t})$ before. I need to
  recheck it more carefully later}
...
\draftnote{Remove the word ``unbiased'' about the estimator \smc[2]-\ds.
  Though I can find a reference for it, as the comment suggested we need to be
  careful about claiming it while using adaptive algorithms, which is not
  introduced yet. So it might be too confusing to state that fact here yet too
  imprecise not to mentioned it}
...
\smc[1]. The estimator of normalizing constant in \smc[3] follows in exactly
the same manner as in the \smc[2] case. In \smc[3], the same estimator
...
\emph{conditional} \ess (\cess) (see Appendix~\ref{sec:Conditinal effective
  sample size} for details of its derivation),
\begin{equation}
...
acceptance rate thresholds and to alter the proposal scale from the simply
estimated value whenever the acceptance rate falls outside those threshold
values.
...
for a fixed $h>0$,
\begin{equation}
  X_t\sim\calN\Round[Big]{(X_{t-1}+\frac{1}{2}\nabla\log\pi(X_{t-1}), hI_d}
\end{equation}
...
optimal scalings. We could also use the particle approximation at time $t-1$
to estimate the covariance matrix of $\pi_{t}$ and thus tune the scale $h$
on-line. As these algorithms are known to be somewhat sensitive to scaling,
and we seek approaches robust enough to employ with little user intervention,
...
vary considerably due to the variety of the data sets, which is expected,
for one of the data sets, the parameter $\nu$ fails to move at all. Considering
...
justification \cite{Beskos:2013vx} -- something which can significantly
complicate the development of effective, theoretically justified schemes in
the \mcmc setting.  Alternatively, we can also specify the proposal scales in
a deterministic, but sensible way. Since \smc algorithms are relatively robust
...
\url{https://github.com/zhouyan/vSMCExample} and the library is also discussed
in chapter~\ref{cha:vSMC: A C++ Library for Parallel SMC}.
...
following experiment, we will use the simulated data set, and choose schedules
that perform both well and poorly for this particular time series. The
