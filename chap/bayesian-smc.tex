...
directly from a complex target distribution, related yet simpler distributions
...
many years and widely used in many fields. Until recently there has been
little interest in using it for Bayesian model comparison for a few reasons.
...
distributions that are difficult for \mcmc algorithms, especially for high
dimensional multimodal distributions. Though it is not impossible to design
\mcmc algorithms for the same problems, it can be hugely difficult in
...
$\varphi_t$ is some function of interest.
...
iteration $t$ or adaptively based on some criteria of the discrepancy bewteen the
...
systematic resampling is performed using the residuals as weights. It has the
...
have a common drawback. They require the knowledge of all the weights being
...
\cite{Chopin:2002hg}. The number of data points to be incorporated in each
...
\mu_t(x_t)$ for some distribution $\mu_t$ at each iteration. Usually,
...
with respect to the sequence of transition kernels $\{K_t\}_{t=1}^T$. Let
...
likely to be large even when the kernel $K_t$ mixes fast. Indeed, later we
will show empirically that, it is preferable to use more distributions rather
than using multiple passes of \mcmc moves in a single iteration, provided
that they use the same computational resources.
...
\begin{enumerate}
  ...
  \item Calculate pairwise evidence ratios, the Bayes factor $B_{k_1k_2}$ for
    two models $\calM_{k_1}$ and $\calM_{k_2}$ directly.
\end{enumerate}
...
\begin{equation}
  ...
  \label{eq:geometry_1}
\end{equation}
for some monotonically increasing $\alpha:[0,1]\to[0,1]$ with $\alpha(0)
...
distributions, but one option is to use for each model $\calM_k\in\calM$,
...
estimator, termed \smc[2]-\ds (see also Equation~\eqref{eq:ratio
  normalized}), given by,
\begin{equation}
  ...
  \label{eq:smc2-ds}
\end{equation}
where $W_{t-1}^{(k,i)}$ is the normalized importance weight the $i$\xth
particle during iteration $t-1$ for model $\calM_k$.
An alternative approach to computing the evidence is also worthy of
consideration. As has been suggested, and shown to perform well empirically
previously \cite{Johansen:2006wm}, it is possible to use all of the samples
from every generation of an \smc sampler to approximate the path sampling
estimator and hence to obtain an estimate of the ratio of normalizing
constants. Section~\ref{sub:Path Sampling via smc2/smc3} provides details for
the use of path sampling for both this and other \smc algorithms discussed
later.
...
with countable collections of models (although this is not much a substantial
...
Given a finite collection of models $\{\calM_k\}_{k\in\calK}$, suppose the
...
\end{align*}
where $(\calM,\theta)$ denote a random variable on the disjoint union space $(\{\calM_k\}\times\Theta_k)\cup(\{\calM_{k+1}\}\times\Theta_{k+1})$.
...
$(\{\calM_k\}\times\Theta_k\})\cup(\{\calM_{k+1}\}\times\Theta_{k+1}\})$, with the sequence of
...
iteration $t$ we can obtain an estimate of the expectation within the integral for
...
directly, the Bayes factor via this approach. In general, given an increasing sequence
...
statistics such as the reduction of variances will be based on simulation for
all the real data sets. The purpose of the current work is to advocate robust
...
and so we can use higher order integration schemes. We can also use
...
\end{equation}
where $\{W_{t-1}^{(i)}\}_{i=1}^N$ denotes the \emph{normalized
  weights} at the end of time $t - 1$, and $\{w_t^{(i)}\}_{i=1}^N$ denotes the
\emph{unnormalized} incremental weights during iteration $t$. 
...
times \cite{DelMoral:2012jq} but it is rather less apparent that it is
...
\emph{target} distributions unless resampling is conducted after every iteration as will be
demonstrated below.
...
and target $\pi_t$ is defined as \cite{Kong:1994ul},
\begin{equation}
  \text{Exact }\ess_t =
...
last resampling epoch. A new quantity, termed \emph{\cess} later, proposed in this work instead uses a weighted
...
The approximation leading to the \cess, given samples
...
In this section, this will now be further explored in a more complex model, a
...
\subsection{\protect\pet compartmental model}
...
In the particular case of the posterior schedule, it is not surprised that
