...
It was introduced in \cite{Liu:1998iu}. In this approach, for $i = 1,\dots,N$,
we have
\begin{equation}
...
Let $Q$ denote the inverse of the cumulative distribution of a multinomial
distribution with size $N$ and eights $(W_1,\dots,W_N)$, the stratified
...
Both the stratified and systematic resampling algorithms can be used together
with the residual resampling. It operates by first compute the integer part
and residuals of $NW_t^{(i)}$, and then stratified or systematic resampling
are performed using the residuals as weights. It has the advantage that the
...
general result given in section~\ref{sec:proof of proposition 1} which could
...
%% $\{\widehat\eta_t\}_{t\ge0}$, on increasing spaces of
...
%% $\widehat\eta_{n-1}$ is updated with Markov transition $M_n$ to $\eta_n =
%% \widehat\eta_{n-1}M_n$, and the resampling step, if multinomial resampling is
%% applied, update the distribution $\eta_n$ to $\widehat\eta_n = \Psi_n(\eta_n)$,
%% where
%% \begin{equation}
...
%% sequence of distributions on increasing dimensions $\widetilde\pi_n$ is
%% interpreted as the flow $\widehat\eta_n$. The Markov transition $M_n$ can be
...
%% With a particular \smc algorithm output, $\widehat\eta_n$ is approximated by the
%% empirical measure, $\widehat\eta_n(\diff x) \approx \frac{1}{N}\sum_{i=1}^N
%% \delta_{X_n^{(i)}}(\diff x)$
...
\begin{proposition}\label{prop:path_clt}
  ...
  each iteration, the path sampling estimator, $\widehat\Xi_{T}^{N}$, as
  ...
  bounded:
  \begin{equation}
    \lim_{N\to\infty}\sqrt{N}(\widehat\Xi_{T}^{N} - \Xi_T)
  ...
  %%   \sum_{i=j}^T\beta_i\frac{\widetilde\pi_{i,j}(X_j)}{\widetilde\pi_{j,j}(X_j)}
  %%   \int\widetilde\pi_{i|j}(x_i|X_j)(\xi_i(x_i) - \pi_i(\xi_i))\intd x_i
  %%   \biggr)
  %% \end{equation}
  %% where $\widetilde{\pi}_{i,j}$ is the $j^{\text{th}}$ coordinate marginal of
  %% $\widetilde{\pi}_i$ and we adopt the conventions that $\widetilde{\pi}_{j|j}
  %% = \mathsf{Id}$ .
...
$\widehat\eta_0$ and transitions $M_t$) evolving in the presence of a
(time-varying) potential (described by $G_t$) such that the marginal law of
the $t^\text{th}$ coordinate is:
\begin{equation*}
  \widehat\eta_t(A) = \frac
  {
    \int_{E_1 \times \ldots \times E_{t-1} \times A}
    \widehat\eta_0(d\widetilde{x}_0)
    \prod_{i=1}^t M_{i}(\widetilde{x}_{i-1},d\widetilde{x}_i)
    G(\widetilde{x}_i)
  }{
    \int_{E_1 \times \ldots \times E_{t}}
    \widehat\eta_0(d\widetilde{x}^\prime_0)
    \prod_{i=1}^t M_{i}(\widetilde{x}^\prime_{i-1},d\widetilde{x}^\prime_i)
    G(\widetilde{x}^\prime_i)
  }
\end{equation*}
for any measurable set $A$.

It is convenient to define the operator $\widehat\Phi_t(\eta)(\diff
\widetilde{x}_t) = G_t(\widetilde{x}_t)\eta M_t(\diff \widetilde{x}_t)/\eta
M_t(G_t)$ which allows us to write, recursively, $\widehat\eta_t =
\widehat\Phi_t(\widehat\eta_{t-1})$ and to define the intermediate
distributions $\eta_t = \widehat\eta_{t-1}M_t$ such that $\widehat\eta_t(\diff
\widetilde{x}_t) = G_t(\widetilde{x}_t) \eta_t(d\widetilde{x}_t) /
\eta_t(G_t)$.
...
$M_{t}(\widetilde{x}_{t-1},d\widetilde{x}_t) =
\delta_{\widetilde{x}_{t-1}}(d\widetilde{x}_{t,1:t-1})
K_t(\widetilde{x}_{t,t-1},d\widetilde{x}_t)$ and $G_t(\widetilde{x}_t) =
\pi_{t}(\widetilde{x}_{t,t-1}) / \pi_{t-1}(\widetilde{x}_{t,t-1})$.

To provide symmetry between the simulation system and the ideal system which
it targets, it is convenient to let $\widetilde{X}_t^i$ denote the extended
...
(i.e., $\widetilde{X}_t^i$ corresponds to the particle system obtained by
...
$$\widehat\eta_t^N(d\widetilde{x}_t) = \sum_{i=1}^N
\frac{G_t(\widetilde{X}_t^i)}{\sum_{j=1}^N G_t(\widetilde{X}_t^j)}
\delta_{\widetilde{X}_t^i}(d\widetilde{x}_t).$$ We refer the reader to
...
\begin{proposition}\label{prop:gen_clt}
  ...
  $\{\widehat\eta_t\}_{t=0}^T$, with the application of multinomial resampling
  ...
  described above it is required that $\xi_t(\widetilde{x}_t) =
  \xi_t(\widetilde{x}_{t,t})$):
  \begin{equation}
    \lim_{N\to\infty}\sqrt{N}
    \sum_{t=0}^T\beta_t(\widehat\eta_t^N(\xi_t)-\widehat\eta_t(\xi_t))
    \xrightarrow{D}\calN(0,V_T(\xi_{0:T}))
  \end{equation}
  where $V_t$, $0\le t \le T$ is defined by the following recursion:
  \begin{align}
    V_0(\xi_0) =& \beta_0 \int \widehat\eta_0(x_0)(\xi_0(x_0) -
    \eta_0(\xi_0))^2 dx_0\notag\\
    V_t(\xi_{0:t}) =& V_{t-1}\biggl(\xi_{0:t-2}, \xi_{t-1}
    + \frac{\beta_t}{\beta_{t-1}}
    \frac{M_t(G_t[\xi_t-\widehat\eta_t(\xi_t)])}{\widehat\eta_{t-1}M_t(G_t)}\biggr)
    + \beta_t^2\widehat\eta_t\left(\frac{G_t(\cdot) (\xi_t(\cdot)-\widehat\eta_t(\xi_t))^2}{\widehat\eta_t(G_t)}\right).
  \end{align}
\end{proposition}
...
$\widehat\eta_t^N(\xi_t)-\widehat\eta_t(\xi_t)$ can be rewritten as
\begin{equation}
  \widehat\eta_t^N(\xi_t)-\widehat\eta_t(\xi_t) =
  \widehat\eta_t^N(\xi_t) - \widehat\Phi_t(\widehat\eta_{t-1}^N)(\xi_t) +
  \widehat\Phi_t(\widehat\eta_{t-1}^N)(\xi_t) - \widehat\eta_t(\xi_t)
\end{equation}
and the weighted sum,
\begin{equation}
  T_t^N(\xi_{0:t}) =\sqrt{N}
  \sum_{j=0}^t\beta_j(\widehat\eta_j^N(\xi_j)-\widehat\eta_j(\xi_j))
\end{equation}
can therefore be written as
\begin{align}
  T_t^N(\xi_{0:t}) &= T_{t-1}^N(\xi_{0:t-1}) +
  \sqrt{N}\beta_t(\widehat\eta_t^N(\xi_t)-\widehat\eta_t(\xi_t)) \notag\\
  &= \bar{T}_t^N(\xi_{0:t}) + \chi_t^N(\xi_t)
\end{align}
where
\begin{align}
  \bar{T}_t^N(\xi_{0:t}) &= T_{t-1}^N(\xi_{0:t-1}) +
  \sqrt{N}\beta_t(\widehat\Phi_t(\widehat\eta_{t-1}^N)(\xi_t) - \widehat\eta_t(\xi_t))
  \label{eq:bar_t_t} \\
  \chi_t^N(\xi_t) &=
  \sqrt{N}\beta_t(\widehat\eta_t^N(\xi_t) - \widehat\Phi_t(\eta_{t-1}^N)(\xi_t))
\end{align}
...
\begin{proof}
  ...
  \begin{align}
    \widehat\Phi(\widehat\eta_{t-1}^N)(\xi_t) - \widehat\eta_t(\xi_t)
    &= \frac{1}{\widehat\eta_{t-1}^NM_t(G_t)}\{
      \widehat\eta_{t-1}^NM_t(G_t\xi_t) -
      \widehat\eta_{t-1}^NM_t(G_t)\widehat\eta_t(\xi_t)\} \notag\\
    &= \frac{1}{\widehat\eta_{t-1}^NM_t(G_t)}
    \{(\widehat\eta_{t-1}^N-\widehat\eta_{t-1})M_t(G_t[\xi_t-\widehat\eta_t(\xi_t)])\}
  \end{align}
where the final equality is a simple consequence of the fact that for any
integrable test function $\varphi$:
\begin{equation*}
\widehat\eta_{t-1} M_t (G_t \varphi) = \eta_t(G_t) \widehat\eta_t(\varphi) \Rightarrow
\widehat\eta_{t-1} M_t (G_t [\xi_t - \widehat\eta_t(\xi_t)]) = \eta_t(G_t)
\underset{=0}{\underbrace{\widehat\eta_t(\xi_t - \widehat\eta_t(\xi_t))}} .
\end{equation*}
...
Substituting this representation into Equation~\eqref{eq:bar_t_t},
  \begin{align}
    \bar{T}_t^N(\xi_{0:t})
    &= T_{t-1}^N(\xi_{0:t-1}) +
    \frac{\sqrt{N}\beta_t}{\widehat\eta_{t-1}^NM_t(G_t)}
    \{(\widehat\eta_{t-1}^N-\widehat\eta_{t-1})M_t(G_t[\xi_t-\widehat\eta_t(\xi_t)])\}
    \notag\\
    &= T_{t-1}^N\biggl(\xi_{0:t-2},
    \xi_{t-1} + \frac{\beta_t}{\beta_{t-1}}
    \frac{M_t(G_t[\xi_t-\widehat\eta_t(\xi_t)])}{\widehat\eta_{t-1}^NM_t(G_t)}\biggr)
  \end{align}
  The proof is completed by using the result \cite[cf.
  Sec.~7.4.3]{DelMoral:2004ux}, that if $G_t$ is essentially bounded below
  then,
  \begin{equation*}
    \frac{1}{\widehat\eta_{t-1}^NM_t(G_t)} \xrightarrow{p}
    \frac{1}{\widehat\eta_{t-1}M_t(G_t)}
  \end{equation*}
...
\begin{lemma}\label{lem:sampling}
  ...
  \begin{equation}
    \lim_{N\to\infty}\chi_t^N(\xi_t)\xrightarrow{D}\calN(0, \widehat{V}_t(\xi_t))
  \end{equation}
  where
  \begin{equation}
    \widehat{V}_t(\xi_t) = \beta_t^2\widehat\eta_t((\xi_t - \widehat\eta_t(\xi_t))^2)
  \end{equation}
\end{lemma}
...
\begin{proof}
  ...
  \begin{equation}
\sqrt{N}\beta_t
    \sum_{j=1}^N\frac{\xi_t(\widetilde{X}_t^{(j)}) - \widehat\eta_{t-1}^N M_t(\xi_t)}{N}
    = \sum_{j=1}^NU_{t,j}^N
  \end{equation}
  where $U_{t,j}^N = \frac{\beta_t}{\sqrt{N}}\{\xi_t(\widetilde{X}_t^{(j)}) -
    \widehat\eta_{t-1}^N M_t(\xi_t)\}$. Define, recursively, the $\sigma$-algebras
    $\calH_t^N = \calH_{t-1}^N \vee
    \sigma(\{\widetilde{X}_{t}^{(j)}\}_{j=1}^N)$, $\calH_{t-1} =
    \sigma\left(\cup_{N=0}^\infty \calH_{t-1}^N \right)$ and the increasing (in $j$) sequence of
  $\sigma$-algebras $\calH_{t,j}^N =
  \calH_{t-1}\vee\sigma(\{\widetilde{X}_t^{(l)}\}_{l=1}^j)$. It is clear that
  \begin{equation}
  ...
  \begin{align*}
    ...
    &= \frac{\beta_t^2}{N}\{\widehat\eta_{t-1}^N M_t(\xi_t^2) -
      [\widehat\eta_{t-1}^N)(\xi_t)]^2\}
...
That is, for any $0 < u\le1$ and $\varepsilon>0$,
  ...
  \begin{align*}
    ...
    \{\widehat\eta_{t-1}^N M_t(\xi_t^2) - [\widehat\eta_{t-1}^N M_t(\xi_t)]^2\} \\
    &= \beta_t^2     \{\widehat\eta_{t-1}^N M_t(\xi_t^2) - [\widehat\eta_{t-1}^N M_t(\xi_t)]^2\}
  \end{align*}
  ...
  \begin{equation}
    \check{V}_t^N(\xi_t) = \beta_t^2 \{\widehat\eta_{t-1}^N M_t (\xi_t^2) -
      [\widehat\eta_{t-1}^NM_t(\xi_t)]^2\}
  \end{equation}
  and as (again, see \cite[Section 7.4]{DelMoral:2004ux})
  \begin{align*}
    \check{V}_t^N(\xi_t)
    &\xrightarrow{P} \beta_t^2 \{\widehat\eta_{t-1}M_t(\xi_t^2) -
      [\widehat\eta_{t-1}M_t(\xi_t)]^2\}
  \end{align*}
  the proof is completed using Slutzky's lemma and applying \cite[Lemma A2]{Chopin:2004cn} which
  yields that:
\begin{align*}
\lim_{N \rightarrow \infty} \mathcal{X}_t^N \xrightarrow{D} \calN(0,\widehat{V}_t(\xi_t))
\end{align*}
with
\begin{align*}
\widehat{V}_t(\xi_t) =
\check{V}_t\left(\frac{G_t(\cdot)}{\widehat\eta_{t-1}M_t(G_t)} (\xi_t(\cdot) -
\widehat\eta_t(\xi_t)) \right) =
\beta_t^2 \widehat\eta_t\left(
\frac{G_t(\cdot)}{\widehat\eta_{t-1}M_t(G_t)} (\xi_t(\cdot) -
\widehat\eta_t(\xi_t))
\right)
\end{align*}
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \Exp[(A_t - \exp(-s^2\widehat{V}_t(\xi_t)/2))B_t]
    +\exp(-s^2\widehat{V}_t(\xi_t)/2)\Exp[B_t]
  \end{align*}
  where $A_t = \Exp[\exp(is\chi_t^N(\xi_t))|\calH_{t-1}^N]$ and $B_t =
  \exp(is\bar{T}^N(\xi_{0:t}))$. The first term can easily be shown to converge
  a.s. to zero as $N\to\infty$ by the asymptotic normality of $\xi_t^N$ and
  the conditional independence of the particles at iteration $t$ given
  $\mathcal{H}_{t-1}^N$.
  The second term is the product of two Gaussian
  characteristic functions and thus we have that $T_t^N$ also follows a
  Gaussian distribution (see the argument of \cite{Kuensch:2005} shown in more detail in Lemma 10{} in
  \cite{Johansen:2006iv}, which uses essentially the same argument, for details).
\end{proof}
...
roughly the same in terms of computational resources as that of the \smc[1] and
\rjmcmc algorithms. From the results of \rjmcmc and \smc[1], we consider four
and five components models (i.e., the true model and the most competitive
...
computation time. As outlined in section~\ref{ssub:Improved Univariate
...
those obtained by a  previous study of the same data \cite{Zhou2013},
...
\smc[2] framework. However, much less tuning is required because the algorithm
