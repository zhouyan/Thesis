...
these information criteria takes the form,
\begin{equation}
...
\label{sub:Consistency and efficiency}
...
formulas of each model selection methods. In \cite{Sin:1996vs}, a throughout
treatment of properties of information criteria can be found. Two properties
are of particular interest, the consistency and efficiency of a given method.

By consistency, we mean a model selector shall pickup the true model if it
exits and is among the candidate models. If we only assume that there is a
true data generating mechanism, which is not among the candidate models, then
the model selector shall choose the model that is closest to this true model
given enough data. There are different ways of measuring this ``closeness''.
The \kl is the most commonly used and the one we considered here. When more
than one models minimize the \kl, then consistency means that the model
selection methods shall choose the model with the least complexity, usually
meaning the shortest parameter vector. More formal treatment can be found in
\cite{Sin:1996vs} and \cite[][sec.~4.1 and~4.3]{Claeskens:2008tq}.
...
when the sample size goes to infinity, the minimal \mse (or other criteria) is
reached. It shall be noted that, the efficiency of model selection methods can
only be studied in context. A method that is efficient for a linear regression
is not necessarily efficient for a time series study.
