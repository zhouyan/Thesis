...
\draftnote{Add a little more details and references on how to use vanilla \mc
  for estimating marginal likelihood.}
...
For some situations, the accuracy can be improved through importance sampling
...
\draftnote{Make it clear that the weak Markov property shown here is the
  special case for time homogeneous chains. And I think it is fine not to
  include more general results since we already said that we are most
...
\draftnote{Fix the description of aperiodicity. Make it clear that the
  description was only an intuition of a sufficient but not necessary
  condition, which is the same way we will use to reason that
  Metropolis-Hastings and others are aperiodic.}
...
\draftnote{Make it clear that we introduce atom and small set here because we
  need them to formally define aperiodicity and ergodicity later}
...
\draftnote{Add the missing irreducibility condition for positivity}
...
\draftnote{Fix the description of uniform ergodicity. Originally there was a
  typo saying $r$ is the same for all $x$, which is clearly untrue. I
  intended to say $r^{-n}M(x)$, but it seems unusual to call it the
  convergence rate. So I settle with just say the convergence speed is the
  same.}
...
\draftnote{I am not quite sure about the comment here. Unless I am mistaken,
  the expectation of the accept rate is taken with respect to $X\sim\pi$
  (given a stationary chain) and $Y\sim\eta$, as written in the originally
  equation. I added some text surrounding the equation and mention that
  $\bar\alpha$ measures how often it is accepted in the long run. Hope it is
  clear now.}
...
\draftnote{Add a minimal description on the algorithm. The model and the prior
...
\draftnote{Still to find some more recent general results as references}
...
\draftnote{Make it clear \rjmcmc does not characterizing small probability
  model well is not necessarily a problem. It is the fact that we cannot be
  sure that the low acceptance rate is a resulting of poor proposal or small
  probability a problem.}
...
\draftnote{Add one sentence saying that these diagnostics are appealing
  because they directly measure the stability of the estimator of interest}
...
\draftnote{Not sure that the scenarios described here about independent MH is
  plausible}
