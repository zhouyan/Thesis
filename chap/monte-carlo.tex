...
Bayes factor. Similar problems were also shown in \cite{McCulloch:1991hj}.
...
In general, the performance of the importance sampling depends not only on the
...
\draftnote{Add a short paragraph saying that this section is somewhat more
  technical than others to allow more compact presentations. I tried to
  rewrite this section with lower level of technicality. However, by assuming
  continuous distributions as the rest of the thesis, I found that I need to
  introduce more notations and making tings, at least notation wise, more
  complicated. Can I move the formal definitions of reducibility etc. into an
  appendix and only keep the more descriptive and intuitive paragraphs here.
  I think it is easier to just keep those materials left here at a lower level
  of technicality}
...
required by later sections. Throughout this thesis, in most places we assumed
that the distributions and random variables under discussion are continuous,
for the purpose of simplicity. This section is somewhat more technical than
others. We found that it is easier to present the theoretical results in a
more compact form with the language of measure theory. However, more intuitive
interpretations of the results are also given wherever it is appropriate.

A Markov chain can be defined in terms of \emph{transition kernels}. In
general, consider a measurable space, say $(E,\calE)$, a transition kernel $K$
is a function defined on $E\times\calE$ such that $K(x,\cdot)$ is a
probability measure for every $x\in E$ and $K(\cdot,A)$ is measurable for
every $A\in\calE$. In the continuous case, we also call the corresponding
conditional density \emph{kernel}, denoted by $K(x,x')$, $x,x'\in E$. That is,
$\Pr(X\in A|x) = \int_A K(x,x')\intd x'$. In this thesis, we are most
concerned with this type of kernels as it is the cases of examples used later.
...
$(x^{t-1},\dots,x^0)$, $X^t$ has the same distribution as it has conditional on
$x^{t-1}$. Clearly a transition kernel is such a conditional distribution. In
the context of \mcmc, we are mostly concerned with \emph{time homogeneous}
...
distribution of $X^0$, $\mu$, and every $(t+1)$ samples $(X^0,\dots,X^t)$, and
some measurable function $\varphi$,
\begin{equation}
  \Exp_{\mu}[\varphi(X^{t+1},X^{t+2},\dots)|x^0,\dots,x_n] =
  \Exp_{x^t}[\varphi(X^1,X^2,\dots)],
\end{equation}
where $\Exp_{\mu}$ denotes expectation with respect to the law of the chain
given the initial distribution of $X^0$ and $\Exp_{x^t}$ denote expectation
with respect to the law of the chain conditional on $X^t = x^t$.
...
chain will visit any state for infinite times. In other words, the Markov chain
...
A Markov chain $(X^t)$ with transition kernel $K$ on $(E,\calE)$ is said to be
$\psi$-\emph{irreducible}, for a given measure $\psi$, if for every
$A\in\calE$ such that $\psi(A)>0$, there exists $t$ such that $K^t(x,A)>0$ for
...
conditional on the initial state $x$. In other words, the probability of reach
any set $A$ in finite many steps is positive.

Two related concepts, \emph{atom} and \emph{small set}, are useful for
formally defining aperiodicity and ergodicity later. A Markov chain $(X^t)$ is
...
\end{equation*}
A chain is \emph{aperiodic} if $d = 1$. If there exists a small set $C$ and an
associated measure $\eta_1$ such that $\eta_1(C) > 0$, that is it is possible
to go from $C$ to $C$ in a single step, the chain is said to be \emph{strongly
  aperiodic}.
...
the space, which is clearly an undesired property for an \mcmc algorithm. It
...
a invariant probability measure $\pi$ of a Markov chain $(X^t)$, if
...
\draftnote{Add a simple derivation of the probability expression of the
...
used as a rule of thumb in practice. Some more recent results for other
proposal distributions including non-Gaussian cases can be found in, e.g.,
...
Using 10,000 iterations as the burn-in period for adaptation, we were able to
...
\sidenote{I checked that the acceptance probabilities of RJMCMC is correct.
  And I added a short paragraph to make it clear that these moves are usually
  designed in pairs}
...
\draftnote{Correct the sampling distribution of Gibbs sampling for the IS
  estimator. And note that though the samples are correlated, the weighted
  terms are not. In addition, make it clear that the proposal distribution is
  time dependent}
