...
the reference manual. The remaining of this section is structured according to
...
from a implementation perspective, they be dissembled into the following
steps.
\begin{enumerate}
...
and need to calculate both the path sampling estimator and other importance
sampling estimates. In the \vsmc library, all these tasks are performed
through the \cppinline{Sampler} class. Below is an example of such a program,
...
\begin{description}
  ...
  \item[\cppinline{mcmc_f1} and \cppinline{mcmc_f2}] Update the particles. For
    ...
    performed after the possible resampling.
  \item[\cppinline{path_eval}] Evaluate the value of path sampling integrands,
    $\diff\log q_{\alpha}(X)/\diff\alpha$.
  \item[\cppinline{moments_eval}] Evaluate importance sampling estimate
    integrands, for example moments.
\end{description}
In addition, in the first line of the program, the class template
\cppinline{Sampler} has a template parameter \cppinline{T}, which is a user
defined value collection type that abstract $\{X^{(i)}\}_{i=1}^N$.

\subsection{The value collection, the particles and the sampler}
\label{sub:The value collection, the particles and the sampler}

At the core of each implementation of \smc algorithms using the \vsmc library
is the definition of the value collection type, which abstract the 

\subsection{Initialize particles}
\label{sub:Initialize particles}

\subsection{Updating particles}
\label{sub:Updating particles}

\subsection{Path sampling}
\label{sub:Path sampling}
...
\subsubsection{The value collections}
\label{ssub:The value collections}
...
memory or even in a database. Users can define their own value collection type
to fulfill various application specific needs. For full details on the
requirement of the value collection type, see \cite{vsmcjss}.

The \vsmc library provides some readily usable implementations. Here we
introduce one of them that shall be sufficient for most applications. Many
typical problems' value collections can be viewed as a matrix of certain type.
For example, a simple particle filter whose state is a vector of length
...
\cppinline{Dim - 1}. To obtain the value at position \cppinline{pos} of the
vector of particle \cppinline{i},
\begin{cppcode}
...
Given a value collection type \cppinline{T}, \cppinline{Particle<T>} defines a
particle collection which abstract both the state values and weights. The
value collection sub-object can be obtained through the
\cppinline{Particle<T>::value} member function.
...
\section{Using the core module}
\label{sec:Using the core module}

In this section, we use examples to demonstrate how to use the features
introduced in section~\ref{sub:Core module}.

\subsection{Example: Simulation of Normal distribution}
\label{sub:Example: Simulation of Normal distribution}

\subsection{Example: Simple particle filter}
\label{sub:Example: Simple particle filter}

\section{Using the SMP module}
\label{sec:Using the SMP module}

In this section, we use examples to demonstrate how to use the features
introduced in section~\ref{sub:SMP module}.

\section{Case study: Gaussian mixture model}
\label{sec:Case study: Gaussian mixture model}
...
\subsubsection{Functors and lambda expressions}
\label{ssub:Functors and lambda expressions}
...
\cppfile{code/sort_f_erasure.cpp}
