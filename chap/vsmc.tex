...
For those interested, Appendix~\appref{sec:Modern C++} serves as a brief
...
require more computations. To solve larger problems without use significantly
longer computing time in the foreseeable future, the only way is to use
parallelism.

Parallel computing is also much more economically efficient in both power consumption and
...
more computing work done with the same or less amount of or time.
...
work is parallelized \cite{parallel}. In practice, one is most concerned with
...
Implementations on different hardwares often concern one of the
...
Another difficulty of parallel computing is the tuning and portability of
performance. Since the development of languages such as Fortran and C,
...
vendors even though they have similar raw computational power.
...
here. Most of them can be characterized by three aspects -- \emph{application areas}, \emph{software environments} and \emph{implementation level}.
...
acyclic graph (\dag) and the data in a similar language. The software
analyzes the model and choose \mcmc algorithms to do the sampling. It is a
easy to use practical tool for Bayesian analysis. The output of \bugs is
...
and it can take long time to get reasonable results. In particular, it is
...
scales for Metropolis random walk algorithms, and even if the user had a good
...
while only a little more programming effort is required. Both \pymc and \cppbugs
give users access to a general programming language in the process of
designing the algorithms. Much flexibility and better performance are gained through
this compared to \bugs. On the other hand, to use them to their full
potentiality, the user does need to have a good knowledge of the programming
languages.
...
class of problems instead of providing a more general framework for
...
more general than others such as the various \rlang packages targeting
specific models.
...
always straightforward. There is demand for softwares that help researchers to
...
However, there are few softwares for the purpose of implementation of
generic \smc algorithms in parallel computers. The \libbi library is a notable
exception.

There is also some fundamental work done in this area. One more important
...
exists between $x_{t+1}$ and $x_t$, which prevents scalable parallelization.
...
We believe there is demand of softwares similar to \smctc, which provides a
...
classes and functions of the library reside in the namespace
...
(\gpgpu). \opencl is a framework for writing programs that can be executed
...
is beyond the scope of this chapter to document most of them. However, a few of them play central roles in the implementation of \smc algorithms.
...
abstracts the collection of values $\{X^{(i)}\}_{i=1}^N$. The library allows
much flexibility in the definition of this type. The important thing to note
here is that this class needs to at least abstract the whole collection of all
...
the importance weights. Each can be accessed as the following,
\begin{cppcode}
Sampler<T> sampler(N);
...
vector of length \cppinline{Dim}, indexed from \cppinline{0} to \cppinline{Dim - 1}. To obtain the value at position \cppinline{j} of the vector of
...
First the number of components is set through a template parameter
...
implementation of \cppinline{init_f}. However, by replacing
...
only \cppinline{InitializeOMP} needs to be changed to
...
done is to wrap the value collection type with the adapter class
\cppinline{StateMPI},
\begin{cppcode}
...
\end{cppcode}
The second argument, a boolean value, indicates whether the new
\cppinline{move} should be appended to the existing (possibly empty) queue (if
...
Recall Algorithm~\ref{alg:smc2}, the updating of weights should be performed
...
estimate, one needs to define the following evaluation function (or other kinds
of callable objects),
...
\cppinline{res[i * dim + j]} should be $h_j(X^{(i)})$.
...
sampling estimator expressions. But the implementation is similar.
...
of length the dimension of the monitor. The function call needs only to store
...
Unlike modern \cpu{}s, which have the same performance for double and single
...
\gpu{}s penalize double precision performance heavily.

For different number of particles, the wall clock time and speedup are
...
implementation. With a large number of particles, \aocl has performance similar to that of the \tbb implementation. \iocl is about 40\% faster than the
...
220\% speedup and the single precision performance has nearly 1600\% speedup.
...
the efforts needed to develop new algorithms, should also be taken into
consideration. Due to the low level natural of \cpp, it certainly takes more
effort to develop an algorithm using \vsmc than, say \libbi or \biips.
...
\begin{enumerate}
  \item Many parallel programming models do not coexist in the same program
    ...
    \openmp. Much less so can be said for the others. Often, some other part of
  ...
  \item Often an algorithm is first developed on a desktop or laptop with only
    multicore processors. Later it may be deployed on larger computers to
...
\vsmc using roughly the same number of lines of code. Considering that there is a
...
increase of effort when compared to the 500 lines \smp implementation. Less
...
is not the case when using \vsmc. The library provides facilities to manage
...
considerable speedup compared to serialized implementations. The \opencl
