...
Another significant benefit of templates is due to the \cpp feature of
...
The first definition of the class \cppinline{SizeTypeTrait} is generic and the
...
The example given above shows how one can modify \cppinline{Sampler}'s
...
function and a pointer, \cppinline{f_ptr}, that points to it,
\begin{cppcode}
...
\cppinline{g}, usually defined in a library that the user is using, requires a
...
\end{cppcode}
It is clear that we cannot use \cppinline{normal_pdf} direclty with
\cppinline{prob}. What we want is in context, to be able to write something
like,
\cppfile{code/callable_update_mcmc.cpp}
The callback \cppinline{target}, defined somewhere else, shall accept a single
value as its parameter yet it shall return \cppinline{normal_pdf(x, mean, sd)}
with \cppinline{mean} and \cppinline{sd} take values of the arguments of
\cppinline{update_mcmc}. This problem can be solved using one of the three
kinds of callable objects introduced in this section,
\cppfile{code/callable_update_mcmc_impl.cpp}
...
\cppinline{g} is perhaps an typo. The type erasure in \cppoo{},
...
\cppinline{build} directory), invoking \shinline{make docs} will create a
\shinline{doc} directory inside \shinline{build}, which contains the \html
...
classes defined in \cppoo{} \cppinline{<random>} or their \boost equivalents
...
\cppinline{<functional>} and \cppinline{<random>}. To instruct \vsmc to use
...
same for each variable. This is similar to \rlang \cppinline{data.frame}.  For
example, suppose each particle's state is formed by two \cppinline{double}'s,
an \cppinline{int} and a user defined type \cppinline{StateType}, then the
following constructs a value collection using \cppinline{StateTuple},
\begin{cppcode}
...
A constant reference to the \cppinline{Particle<T>} object can be obtained
through \cppinline{sp.particle()}. And \cppinline{sp.particle_ptr()} returns
a pointer to such a constant \cppinline{Particle<T>} object. Note that, one
...
instantiation or its derived class, then calling \cppinline{sp.state(pos)} is
equivalent to calling \cppinline{particle.value().state(i, pos)} and the
reference it returns is mutable. See the reference manual for more
informations on the interface of the \cppinline{SingleParticle} class
template. Note that, these \cppinline{SingleParticle<T>} objects are usually
