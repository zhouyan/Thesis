...
However, most of today's popular statistical softwares are written with
...
It is followed by a description some of the commonly seen of parallel
computers. Then we discuss the importance and limitations of parallel
computing in general. This section is concluded with a discussion of its use
in the particular field of Monte Carlo algorithms.
...
problem size grows. For example, parallelizing a vanilla algorithm belongs to
...
discussed later. In contrast, an \mcmc algorithm usually cannot be
parallelized in a scalable way. To obtain better statistical results, often
the only way is to increase the number of iterations, and thus no matter how
much parallel computing resources are available, the computing time will
increase without increasing of the speed of the processors.
...
For example, say a program performs functions $f_1,\dots,f_k$, then at best the
computing time can be reduced by $k$-fold through parallelism. In the
...
ideas from the encryption literature, where for an integer $x$, a number $y$ is
generated through a deterministic, one-to-one mapping, $y = f(x)$. This
...
algorithms cannot be efficiently parallelized. The \smc algorithms, though many
...
\section{Introducing some advanced C++ techniques}
\label{sec:Introducing some advanced C++ techniques}
...
function and a pointer, \cppinline{f_ptr}, that point to it,
\begin{cppcode}
...
There are a few drawbacks of using function pointers to pass callbacks.
Because it can only points to free functions, which are defined in the
...
with \cppinline{operator()} can be used with the same syntax as a function.
For example,
\begin{cppcode}
struct C
{
...
advantage of object-oriented programming. The class can have constructors and
data members just as any other classes. It can also be used as a way of
passing callbacks, similar to function pointers, through templates. For
instance,
\begin{cppcode}
template <typename F>
...
g(3.14, c_obj);
\end{cppcode}
The new definition of function \cppinline{g} accepts both the function pointer
and the class object as its argument.
...
example of using the function template \cppinline{g} defined before with
lambda expression,
\begin{cppcode}
...
-> /* return type */ { /* body * };
\end{cppcode}
The \cppinline{/* mutable */} part can be either empty of the keyword
...
from the body. And if the body does contain any \cppinline{return} statement, it
is deduced to be \cppinline{void}. If the expression takes no arguments, the
parameter list can also be omitted.
...
\begin{description}
  \item[\cppinline{[a, &b]}] captures \cppinline{a} by value and \cppinline{b}
    by reference.
  \item[\cppinline{[this]}] captures the \cppinline{this} pointer.
...
In the previous example, our function \cppinline{f} or class \cppinline{C}
only computes the square of a number. Now consider the problem that we want it
take compute the product of its parameter with an external variable of
function \cppinline{g}. Here is a comparison of the solutions using function
pointers, functors, and lambda expressions,

Using function pointers,
\begin{cppcode}
double A; // A global variable
double f (double x) { return x * A; }

// The following may be in a local scope
double y = 1.414; // A local variable
A = y; // Modify the global variable used by f
g(3.14, &f);
\end{cppcode}
Using functors,
\begin{cppcode}
struct C
{
    ...
    double operator() (double x) const { return x * a_; }
...
g(3.14, C(y));
\end{cppcode}
And the last, using lambda expressions,
\begin{cppcode}
double y = 1.414; // A local variable
g(3.14, [=] (double x) -> double { return x * y });
\end{cppcode}
...
lambda expression is clean, elegant and yet it does not requires the use of
global variables.

\subsubsection{Type erasures}
\label{ssub:Type erasures}

In the example above, the function template \cppinline{g} does not actually
enforce the signature of the function or functor, in contrast to the defintion
of it that takes a function pointer as an argument. For example, the
following is perfect valid \cpp,
\begin{cppcode}
int h (int x) {return x * x};
g(3.14, &h);
\end{cppcode}
while it may be not what one wants. The use of \cppinline{h} with
\cppinline{g} is perhaps an typo. The type erasure through \cppoo{}
\cppinline{std::function} provides a solution to this problem. A \emph{type
  erasure} can convert various types of objects into a single type. Below is
a basic usage of \cppinline{std::function},
\begin{cppcode}
...
\end{cppcode}
Now we can redefine the function \cppinline{g} as,
\begin{cppcode}
